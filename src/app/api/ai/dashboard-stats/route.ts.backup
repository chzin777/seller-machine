  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(notas) && notas.length > 0) {
        // Converter notas para pedidos
        vendas = {
          pedidos: notas.map((nota: any, index: number) => ({
            id: nota.id || index + 1,
            valorTotal: (parseFloat(nota.valorTotal?.toString() || '0') || 0) / 100,
            itens: (nota.itens || []).map((item: any) => ({
              produto: { nome: item.produto?.descricao || `Produto ${(index % 5) + 1}` },
              quantidade: parseFloat(item.Quantidade?.toString() || '1')
            }))
          }))
        }

        // Construir lista de clientes baseada nas notas
        const clienteIds = Array.from(new Set(notas.map((n: any) => n.clienteId).filter((id: number | null) => id)))
        clientes = clienteIds.map((cid: number, i: number) => ({
          id: cid,
          nome: `Cliente ${cid}`,
      ...
      console.warn('Erro ao buscar dados via Prisma para dashboard-stats, usando fallback', prismaError)
    } finally {
      await prisma.$disconnect()
    }
    
    // Se não obtivemos dados via Prisma, tentar APIs externas
    if (vendas.pedidos.length === 0) {
      try {
        ...
        // Buscar dados reais das notas fiscais da API externa
        try {
          ...
          // Fallback final: usar dados da API local de vendedores com headers (respeita escopo de vendedores)
    ...
    // Se nenhuma das opções funcionar, usar dados mock
    if (vendas.pedidos.length === 0) {
