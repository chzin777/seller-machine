...
import { LayoutDashboard, Link2, Users, Menu, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Proteção de rotas: se não estiver logado, redireciona para /login (exceto /login e /cadastro)
  useEffect(() => {
    ...
    <html lang="pt-BR" suppressHydrationWarning className="w-full h-full min-h-screen">
      ...
      <body className="bg-gray-50 text-gray-900 min-h-screen w-full h-full m-0 p-0 dark:bg-gray-900 dark:text-gray-100 font-sans">
        <div className="flex min-h-screen">
          ...
          <aside
            ref={sidebarRef}
            className={`relative transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-16'} bg-gray-50 dark:bg-gray-900 border-r-0 flex flex-col py-6 px-2 gap-8 shadow-xl z-20 ${isLoginPage ? 'hidden' : ''}`}
            aria-label="Menu lateral"
            style={{ boxShadow: sidebarOpen ? '4px 0 24px 0 rgba(0,0,0,0.12)' : undefined }}
          >
            {/* Barra azul vertical */}
            <div className="absolute left-0 top-0 h-full w-1.5 bg-blue-700 rounded-r-lg" style={{opacity:0.85}} />
            {/* Botões de expandir/recolher sempre visíveis para melhor UX */}
            {/* Logo centralizada no topo */}
            <div className={`flex flex-col items-center mb-0 transition-all duration-300 group/sidebar relative px-0`}>
              {sidebarOpen ? (
                <img
                  src="/images/logo.png"
                  alt="Logo Máquina de Vendas"
                  className="block transition-all duration-300 mx-auto"
                  style={{ height: 96, maxWidth: 200, marginBottom: 0 }}
                />
              ) : (
                <img
                  src="/images/logo.png"
                  alt="Logo Máquina de Vendas"
                  className="block transition-all duration-300 mx-auto"
                  style={{ height: 40, maxWidth: 40, marginBottom: 0 }}
                />
              )}
            </div>
              ...
              {navLinks.map((link, idx) => {
                ...
                if (link.type === "toggle") {
                  return (
                    <button
                      key={"toggle-menu"}
                      className={`group flex items-center gap-3 rounded-lg px-3 py-2 font-medium transition-colors hover:bg-blue-700/10 hover:text-blue-700 text-blue-700 dark:text-blue-200 focus:outline-none ${!sidebarOpen ? 'justify-center px-2' : ''}`}
                      style={{ boxShadow: 'none', border: 'none', background: 'none', width: '100%' }}
                      onClick={() => setSidebarOpen(!sidebarOpen)}
                      aria-label={sidebarOpen ? 'Recolher menu' : 'Expandir menu'}
                      tabIndex={0}
                      type="button"
                    >
                      <Icon className={`w-5 h-5 transition-colors group-hover:text-blue-300 hover:cursor-pointer ${sidebarOpen ? 'text-blue-400' : 'text-blue-400'}`} />
                      <span className={`${sidebarOpen ? 'block' : 'hidden'} transition-all duration-300 hover:cursor-pointer`}>{link.label}</span>
                    </button>
                  );
                }
                let isActive = false;
          ...
          <div className="flex-1 flex flex-col min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-white dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
            {/* Topbar */}
            {/* Topbar substituído por Header Client Component */}
            {/* Content */}
            <main
              className={`flex-1 overflow-y-auto ${isLoginPage ? 'p-0 bg-transparent' : 'p-6 sm:p-8 bg-transparent'}`}
              style={isLoginPage ? { background: 'transparent' } : {}}
            >
              <div className={isLoginPage ? 'hidden' : ''}>
                <Header userName={userName} title={
                  pathname === '/ajuda' ? 'Ajuda'
                  : pathname === '/feedback' ? 'Feedback'
                  : pathname === '/configuracoes' ? 'Configurações'
                  : pathname === '/' ? 'Painel de Vendas'
                  : pathname === '/associacoes' ? 'Associações'
              ...
              {!isLoginPage && (
                <footer className="w-full text-center text-xs text-blue-200 mt-8 select-none">© 2025 Seller Machine</footer>
              )}
            </main>
          </div>
        </div>
        <div className={`fixed bottom-6 right-6 z-50 ${isLoginPage ? 'hidden' : ''}`}>
          <ThemeToggle />
        </div>
      </body>
    </html>
